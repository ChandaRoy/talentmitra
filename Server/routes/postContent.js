...
router.get('/my-topic-threads', function (req, res, next) {
  Topic.find({ postedByEmail: req.user.email }).sort('-postedOn')
    .then(data => {
      res.send(data);
    });
  // Topic.find({
...
router.get("/topic/:id", (req, res, next) => {
  ...
  Topic.findById(req.params.id)
    .populate('postedBy', 'firstName lastName photo company aboutMe email')
    .populate('answers.answeredBy', 'firstName lastName photo company aboutMe email')
    .populate('answers.answerComments.commentedBy', 'firstName lastName photo company aboutMe email')
    .then(data => {
      if (data) {
        // console.log(data);
        res.send(data);
...
router.post('/addTopic', function (req, res) {
  let tags = (req.body.tags).map(name => name.toLowerCase());
...
router.post('/topicAnswer', function (req, res) {
  var answerDetails = {
...
router.post('/topicAnswerComment', function (req, res) {
  var commentDetails = {
    "commentText": req.body.commentText,
    "commentedBy": req.user.id,
    "commentedOn": new Date()
  ...
  Topic.findOneAndUpdate({
    ...
    { $push: { "answers.$.answerComments": commentDetails } }
  ).then(data => {
    if (data) {
      console.log(data);
      res.send(data);
...
// Get topics by tags
router.get("/getByTag/:keyword", (req, res, next) => {
  req.params.keyword = (req.params.keyword).toLowerCase();
  Topic.find(
    {tags:{$elemMatch:{$eq:req.params.keyword}}}
  ).then(data => {
    res.send(data);
  });
});
